...
namespace zzs {
	...
	struct Constraint
	{
		...
		Vector2f vConstrainedPos; // the current position of the constraint point

		Constraint(){nVertex = 0; vConstrainedPos = Eigen::Vector2f::Zero();}
		Constraint(size_t nVert, const Vector2f &vPos)
		{
			nVertex = nVert;
	...
	public:
		...
		void buildMesh();
		void buildMesh2(std::vector<Vector3i>& tri);
