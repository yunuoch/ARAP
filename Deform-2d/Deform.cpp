...
void Deform::setAddPoint()
{
	bAddPt = !bAddPt;
}

bool Deform::isAddPoint() const
{
	return bAddPt;
}

void Deform::buildMesh()
{
	...
	{
		vPoints.push_back(Vector2d(m_vInitialVertices[i].vPosition[0], m_vInitialVertices[i].vPosition[1]));
	}
	...
	for (size_t i = 0; i < nTriangle; i++)
	{
		...
		for (size_t j = 0; j < 3; j++)
		{
			...
			Vector2f v0 = m_vInitialVertices[t.nVertices[n0]].vPosition;
			Vector2f v1 = m_vInitialVertices[t.nVertices[n1]].vPosition;
			Vector2f v2 = m_vInitialVertices[t.nVertices[n2]].vPosition;

			// find coordinate system
			Vector2f v01(v1 - v0);
			Vector2f v01N(v01); 
			v01N.normalize();
			Vector2f v01Rot90(v01.y(), -v01.x());
			Vector2f v01Rot90N(v01Rot90); 
			v01Rot90N.normalize();

			// express v2 in coordinate system
			Vector2f vLocal(v2 - v0);
...
void Deform::addPoint(Vertex &v)
{
	m_vInitialVertices.push_back(v);
	m_vDeformedVertices.push_back(v);
}

const std::vector<Vertex> & Deform::getVerts() const
{
	return m_vInitialVertices;
}

const std::vector<Triangle> & Deform::getTriangles() const
{
	return m_vTriangles;
}

const std::vector<Vertex> & Deform::getDeformedVerts() const
{
	return m_vDeformedVertices;
}

size_t Deform::findHitVertex(float x, float y)
{
...
void Deform::setVertex(size_t index, Point position)
{
...
void Deform::updateConstraints(const std::set<size_t>& selected)
{
...
void Deform::updateMesh(bool isRigid)
{
	// perform step 1
	size_t nConstraints = m_vConstraints.size();
	...
	for (size_t i = 0; i < nVerts; i++)
	{
		...
		m_vDeformedVertices[i].vPosition = Vector2f(fX, fY);
...
void zzs::Deform::buildMesh2(vector<Vector3i>& tri)
{
	m_vTriangles.clear();

	//vector<Vector3i> tri = {Vector3i(0,1,2),Vector3i(0,1,3)};
	for (int i = 0; i < tri.size(); i++)
	{
		Triangle t;
		for (int j = 0; j < 3; j++)
		{
			t.nVertices[j] = tri[i][j];
		}
